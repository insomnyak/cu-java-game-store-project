...
public class SalesTaxRateJdbcTemplateDaoImpl implements SalesTaxRateDao{
    ...
    private final String UPDATE_SALES_TAX_RATE_SQL =
            "update sales_tax_rate set state = ?, rate = ? where state = ? and rate = ?";
    ...
    private final String DELETE_SALES_TAX_RATE_SQL =
            "delete from sales_tax_rate where state = ? and rate = ?";
    ...
    @Override
    public List<SalesTaxRate> find(String state) {
        return jdbcTemplate.query(SELECT_SALES_TAX_RATE_SQL, this::mapRowToSalesTaxRate, state);
    }
    ...
    @Override
    public void update(SalesTaxRate salesTaxRate, SalesTaxRate oldSalesTaxRate) {
        jdbcTemplate.update(UPDATE_SALES_TAX_RATE_SQL,
                salesTaxRate.getState(),
                salesTaxRate.getRate(),
                oldSalesTaxRate.getState(),
                oldSalesTaxRate.getRate());
    }

    @Override
    public void delete(SalesTaxRate salesTaxRate) {
        jdbcTemplate.update(DELETE_SALES_TAX_RATE_SQL,
                salesTaxRate.getState(),
                salesTaxRate.getRate());
    }
